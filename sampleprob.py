...
with tf.Session() as session:
    ...
    def generate_samples():
        samples_prob = session.run(fake_inputs)
        samples = np.argmax(samples_prob, axis=2)
        decoded_samples = []
        sample_prob = []
        for i in xrange(len(samples)):
            ...
            for j in xrange(len(samples[i])):
                ...
                print(samples_prob[i][j][samples[i][j]])
                prob *= samples_prob[i][j][samples[i][j]]
            sample_prob.append(prob)
            decoded_samples.append(tuple(decoded))
        return decoded_samples, sample_prob

    def save(samples, sample_probs):
    ...
    for i in xrange(int(args.num_samples / args.batch_size)):
        samples, samples_prob = generate_samples()
        samples.extend(samples)
        sample_probs.extend(samples_prob)
        # append to output file every 1000 batches
        if i % 1000 == 0 and i > 0: 
            
            save(samples,sample_probs)
            samples = [] # flush
